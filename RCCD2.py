...
protein = adata[:,adata.var.feature_types.isin(["Antibody Capture"])]
...
adata.uns["marker_quality_before"] = qulity_check(rna[:,rna.uns["rna_marker"]].X,protein[:,protein.uns["protein_marker"]].X)
threshold = np.percentile(adata.uns["marker_quality_before"][0], int(threshold_quality), interpolation='nearest')
train_rna = rna[adata.uns["marker_quality_before"][0]>threshold].X
...
adata.uns["produced_protein"] = gen.predict([rna.X,protein.X])
